...
> 状态：**M0–M7 全部收口**；M8（写入侧复核）代码完成、判别力已实测
> （**J 0.829 / FPR 0.000**，§11.19），**默认开关待定**。
> 日期：2026-08-11
>
> **v0.23 变更**：跑完 M8 8.4 两轮（110 次调用 / 145k token，
> `decide_ab.jsonl` 是最终、`decide_ab_v1.jsonl` 是补盲区前）。
> 第二轮只改了一处提示词、两侧重测：**TPR 0.686 → 0.829，FPR 恒为 0** ——
> 补的是「证据为空时分辨『让失败可观测』与『声称知道原因』」，
> 同族那个证据同样为空的负例没有被误伤，所以动的是判别力不是偏置（§11.9c 的方法奏效）。
...
> `Architect._review_write()`、`bench/decide_ab.py`（11 用例 / 6 种缺陷形态）与
> `cli bench-decide`。**默认关闭是刻意的**：M7 的 J 0.98 是在拆解上测的，
> 判别力不能假设迁移（§11.13 有过判据移植后无可判之物的先例），出口 3 未达成。
...
| 3 | 架构师本身成为单点故障——它的规格拆解错误无人纠正 | ⚠️ **暴露面已量化并收窄**。停止判断已修（§11.9，J 0.12→0.60）；拆解侧三角色到位、跨模型 J 0.98 vs 0.66（§11.11）。执行侧：M2 交叉表显示 61% 的裁决已被确定性下限送到人面前，真实缺口是**改了 spec 且无人过目的 34/176 = 19%**（M8）。M8 的复核链路已实现但**默认关闭 —— 判别力还没测**（出口 3 未达成） | M8 8.4 ⬜ |
...
#### 两条保留，别把 FPR=0 读成「没有误报风险」

1. **负例是构造得偏易的。** 用例表纪律要求负例只用 `added_criteria`
   （不动 goal/scope/上限），那是「改得对」的操作性定义，但也让它们容易辨认。
   真实世界里恰当的改动会包含改 goal 的情形，FPR 在那种输入上没测过。
2. **n=20，且这是第一轮就得到的 0。** 没有为了好看返工过用例表 ——
   §11.11 那条「不能改用例改到 FPR 归零」在这里没有被违反，
   但 0/20 的置信区间上界仍有约 14%，不能当成「零误报」。

#### 默认开还是关

数据支持开：**FPR 0/20，这批样本上一次都没白打扰人**；五种缺陷形态里四种满分
（含最危险的改松目标 10/10），总体 TPR 0.829。成本是每次「改 spec 且未升级」多约
1.5k token，落在 19% 的裁决上。J 0.829 高于本项目已采纳的同类判断
（M5a 停止判断 0.60、M7 拆解侧 deepseek 0.66、kimi 0.98）。

**仍然关着的理由只剩上面那两条**（负例构造偏易、n=20），它们是「别过度解读这个 0」
而不是「这东西不能用」。真开之前想清楚它挡不住什么：**`limit_raised` /
`scope_widened` 这一族需要推断，实测 2/5，且在抖** —— 靠调大上限或扩 scope
绕过问题的改动，一半以上会被放行。

**下一步要提升，仍然先动提示词而不是换模型**（这一轮已经证明这条路有效）。
剩下那一族的难点不同：它要的是从证据里**推断**出「在原地打转」，不是分辨证据性质。
换 kimi 复核也值得一试 —— §11.11 里它在拆解层是 0.98 vs 0.66，
但那不能外推到这一层，得实测。`decide_ab.jsonl` 是现成基线。
...
M8 ◻ 写入侧复核          代码完成、默认关闭。**判别力未测**（出口 3）——
                         跑 `bench-decide` 出 TPR/FPR/J 之前不该默认开
```
...
| 8.4 | 跑对照实验，出 TPR/FPR/J | ✅ §11.19：J 0.686 →（补提示词）**0.829**，FPR 恒 0 |
| 8.5 | 按实测决定默认开关 | ⬜ 待定 —— 数据支持开，见 §11.19 末节 |
...
3. ✅ 两侧都有数据（正例 35 次 / 负例 20 次），**J=0.829、FPR=0.000**（§11.19）；
4. ⬜ 默认开关待定 —— 数据支持开，但负例的构造偏易，见 §11.19 的两条保留。

...
> （J 0.686 / FPR 0.000，§11.19），**默认开关待定**。
> 日期：2026-08-11
>
> **v0.23 变更**：跑完 M8 8.4，新增 §11.19（55 次调用 / 71k token，`decide_ab.jsonl`）。
...
数据支持开：**FPR 0/20 意味着它在这批样本上一次都没有白打扰人**，而它接住了
四种缺陷形态里的全部、总体 2/3；成本是每次「改 spec 且未升级」多约 1.5k token，
落在 19% 的裁决上。J 0.686 与本项目已经采纳的东西同一水位（M5a 停止判断 0.60、
M7 拆解侧 deepseek 0.66）。

保留意见也实在：上面两条，加上**最危险的形态恰好落在盲区里**。
开了之后要清楚它挡不住「证据为空时编因果改目标」这一种 —— 那一种仍然只有人能挡。

**下一步如果要提升，先动提示词而不是换模型**：现在的提示词没有一句话处理
「证据为空」这种情形。加一条「证据为空时，任何声称知道失败原因的改动都要报」
是最便宜的尝试。但按 §11.9c 的规矩，**改提示词必须两侧都测** ——
`decide_ab.jsonl` 现在就是那个基线，重跑一轮 55 次调用即可比较。
...
| 8.4 | 跑对照实验，出 TPR/FPR/J | ✅ 见 §11.19（单臂 deepseek） |
| 8.5 | 按实测决定默认开关 | ⬜ 待定，见 §11.19 的结论 |
...
3. ✅ 两侧都有数据（正例 35 次 / 负例 20 次），J=0.686、FPR=0.000（§11.19）；

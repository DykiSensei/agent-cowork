# 多 Agent 协作系统 — 开发文档 v0.22

> 状态：**M0–M7 全部收口**；M8（写入侧复核）代码完成但**判别力未测、默认关闭**。
> 日期：2026-08-11
>
...
| 8.3 | `bench/decide_ab.py`：11 用例（负例 4 / 正例 7）× 6 种缺陷形态 | ✅ 脚手架就绪 |
| 8.4 | **跑对照实验，出 TPR/FPR/J** | ⬜ **未跑** |

**默认关闭（`Architect(review_writes=False)`）是刻意的。** M7 那个 J 0.98 是在
**拆解**上测的，输入形状和判断类型都不一样，判别力不能假设迁移 —— §11.13 已经
有过先例：「指纹重复」判据移植到拆解层之后**没有可判之物**，判据还在、命中率是零。
按这个项目的规矩，没有对照实验的东西不进控制流的默认路径。

**误报在这一层比在拆解层贵**：拆解层一次假阳性 = 白重生成一轮；这里 = 白打扰人一次，
而且发生在任务跑到一半、人正在做别的事的时候。FPR 要压得比 7.2 更狠。

**里程碑出口**（未达成）：

1. ✅ 只在真实暴露面上花钱（写入且未升级），暴露面本身有实测依据；
2. ✅ 循环与拆解层同构，判据复用 `escalation` / `policy`；
3. ⬜ 两侧都有数据且 J 优于同模型基线 —— **要跑 `bench-decide`**；
4. ⬜ 按实测结果决定默认开关，并更新风险 #3。

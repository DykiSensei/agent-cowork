...
> 日期：2026-08-11
>
> **v0.23 变更**：跑完 M8 8.4 三轮（165 次调用 / 210k token）。
...
数据支持开，用 kimi 复核：**FPR 0/40（两个 arm 各 0/20），一次都没白打扰人**；
六种缺陷形态里五种 ≥4/5（含最危险的改松目标 10/10 与改考题 5/5），
J 0.886 高于本项目已采纳的同类判断（M5a 停止判断 0.60、M7 拆解侧 0.66/0.98 那一档）。
成本是每次「改 spec 且未升级」多约 1.2k token，落在 19% 的裁决上。

**仍然关着的理由只剩两条**（负例构造偏易、n=20），它们是「别过度解读这个 0」
而不是「这东西不能用」。真开之前想清楚 kimi 挡不住什么：`non_responsive` 2/5 ——
但那一种下一轮会被指纹重复的确定性判据接住，是这三种漏报里代价最低的。

**下一步要提升，仍然先动提示词而不是换模型。** 现在两个模型的盲区互补，
一个自然的想法是「两个都问」—— 别做：那会把成本翻倍、把 FPR 叠加，
而 §2.3 的写入决策点也会变得更难说清是谁在拍板。先试着用一句话把 kimi 在
`non_responsive` 上补回来（它现在是三种漏报里最容易补的），
`decide_ab_kimi.jsonl` 是现成基线。
...
| 8.4 | 跑对照实验，出 TPR/FPR/J | ✅ §11.19：三轮 165 次调用。J 0.686 →（补提示词）0.829 →（换 kimi）**0.886**，FPR 恒 0 |
| 8.5 | 按实测决定默认开关 | ⬜ 待定 —— 数据支持开、复核者选 kimi，见 §11.19 末节 |
...
3. ✅ 两侧都有数据、两个 arm 都测（正例各 35 / 负例各 20），
   **J=0.886（kimi）/ 0.829（deepseek），FPR 恒 0**（§11.19）；
4. ⬜ 默认开关待定 —— 数据支持开且复核者选 kimi，但负例的构造偏易，
   见 §11.19 的两条保留。

# 多 Agent 协作系统 — 开发文档 v0.11

> 状态：**M7 全部任务已实现**（7.1–7.5），出口 1/2/4 达成、出口 3 部分达成；下一步 **M6（群聊界面层）**，或先补风险 #17 的样本
> 日期：2026-08-10
>
...
- **生成-复核循环在真实模型上一次都没触发过**（5/5 一轮通过）。机制、上限、测试都在，但重生成路径没有真实样本，`max_regenerate` 的依据仍是结构性的 —— 新增风险 #17。**「测试全绿」和「这条路径被真实跑过」是两件事**
...
| 17 | 生成-复核循环的重生成路径**没有真实模型的样本** | M7 7.4（§11.12）：5 个目标全部一轮通过，循环、两条上限、意见回喂只有脚本后端测试覆盖。`max_regenerate=2` 的依据是结构性的，不是实测的 | 需要会被驳回的目标 |
...
| §12 M7 7.4 生成-复核循环 | `Architect.plan()` + `escalation.deterministic_plan_escalation()` | ⚠️ 机制在，真实模型未触发（风险 #17） |
| §12 M7 7.5 拆解层人的入口 | `HumanGate.review_plan()` + `CliGate` / `AutoApproveGate` | ✅ |

233 个测试。不起 Docker / Postgres 时依赖真实服务的那些 skip，其余照常跑。
...
M7 ✅ 拆解三角色          ← 7.1–7.5 全部实现（§11.11 / §11.12）。出口 3 只做到
                            「机制在」，重生成路径缺真实样本（风险 #17）
M6    群聊界面层          ← 下一步（与 M7 无耦合）
```
...
3. ⚠️ 生成-复核循环有确定性上限，超限升级给人 —— 机制在、测试在，**但真实模型上没触发过**（风险 #17）；

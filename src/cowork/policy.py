...
class Policy:
    ...
    # 这里曾有 §3.4 软信号消费检查点的两个参数（soft_queue_threshold /
    # soft_interval_s：攒够几条、或隔多久，再批量分诊一次）。**已删除** ——
    # 它们唯一的读者 `Architect.should_consume_soft()` 没有任何调用方，
    # orchestrator 在每个检查点无条件消费。M2 实测也说明不值得接上：软信号极稀疏
    # （75 次运行 13 次出现、共 20 条、队列深度最大 2，阈值 5 永远达不到），
    # 分诊总成本占总 token 的 0.3%（§11.6c）。
    # **留下的判据：「有代码读它」不等于「它在起作用」，要一路问到调用链的头。**

...
1. 每个子任务必须有 scope（它被允许写的文件），**两个子任务的 scope 不能相交**——
   ...
   scope 是**文件级**的：写清每个子任务写哪几个文件，**不要用「一个子任务一个
   子目录」来满足不相交**。需要被多方 import 的共享物（入口 index.html / main.py、
   公共模块、构建脚本、package.json 这类）**归一个「集成/入口」子任务写**，写进
   它的 scope；其余子任务只写自己那批文件、不碰共享物。这样既不相交，依赖方也从
   入口 import 得到；
2. depends_on 只能引用本次拆解里的其它 id，不能有环；
3. **至少要有两个子任务能同时开跑**（即存在两个互不依赖的任务）。做不到就说明
   这个目标是顺序依赖的 —— **子任务之间要 import 彼此的产物，就是顺序依赖**
   （前端页面引数据模块、入口引组件、代码引公共模块都是）。顺序依赖强的任务宁可
   只拆成 1 个子任务（一个 agent 从头到尾顺序做完），或拆成一条短顺序链（每步验收
   上一步的产物），**不要硬凑并行** —— 用多 agent 最差会掉 70% 的效果。

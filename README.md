...
实测 J 0.829 / FPR 0.000。它现在接得住「把失败输入从目标里摘出去」这一类，
但**接不住「靠调大步数上限或扩 scope 绕过问题」** —— 那需要从证据里推断出
「在原地打转」，实测只有 2/5 且不稳定（§11.19）。
...
| 项 | 状态 |
|---|---|
| **写入侧复核的默认开关** | M8 出口 5。实测两轮（§11.19）：补完提示词后 **J 0.829 / FPR 0.000**，五种缺陷形态里四种满分（含最危险的改松目标 10/10）。剩下的盲区是「靠调大上限或扩 scope 绕过问题」—— 需要推断，只有 2/5 且在抖。数据支持默认开，保留意见是负例构造偏易、n=20；`review_writes` 目前仍默认关闭 |

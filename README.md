...
423 个测试。三样基础设施都不起时，依赖它们的 14 个会 skip（3 个 Postgres +
...
**成本护栏是粗的。** 现在有一道应用层硬上限（`--budget`，默认 100 万 token，
不依赖 LiteLLM），超了就抛 `BudgetExceeded` —— 和 LiteLLM virtual key 硬拒绝
同一个信号，因此照常走「升级给人」而不是崩掉。但它**最多会超出一次调用的量**
（花钱前无法预知这次要花多少），所以它是「停下来的水位线」不是「总额不会超过」。
要按次预扣仍然得上 LiteLLM。另外架构师本身是系统里最贵的组件
（M2 实测占总 token 的 51.3%，每次介入约 7k）；一次 `plan --run` 打真实模型
10–35k 起步，跑批 1.6M。

**架构师仍然是唯一的写入决策点。** M7 解了拆解那半；
M8 把复核接到了 `decide()` 的写入侧，**默认开**（设置页可关）：26 个用例上
J 0.963（deepseek 复核）/ FPR 0/24，六种缺陷形态里四种 9/9、两种 8/9。
它防的是没有兜底的那两种 —— 目标被改松、scope 扩到校验脚本 —— 那两种会让任务
**假装成功**。但**它的重做循环没在真实链路上测过**（对照实验故意绕开了那一段），
默认开之后才第一次跑（§11.19）。
...
| 项 | 状态 |
|---|---|
| **写入侧复核的重做循环** | M8 剩下的唯一未知项：判别力测了四轮 321 次调用（26 用例，deepseek J 0.963 / FPR 0/24），但「复核驳回 → 架构师重做 → 第二版更好吗」这一段真实模型上没跑过 —— 对照实验故意绕开了它。默认开之后它才上真实链路（§11.19） |
| ~~模型选型~~ | 已定 deepseek。**这个结论翻过一次** —— 11 个用例时看着是 kimi 更好，每种缺陷形态扩到 3 个用例后反了过来。一种形态一个用例时，用例难度和形态难度分不开 |

...
实测 J 0.686 / FPR 0.000 —— 但**最危险的那种形态它接不住**：证据为空时架构师
编一个因果、据此把目标改松，复核者手上没有东西可以对质（§11.19）。
...
| 项 | 状态 |
|---|---|
| **写入侧复核的默认开关** | M8 出口 5。实测已做（§11.19，J 0.686 / FPR 0.000）：四种缺陷形态满分，但**「证据为空时编个因果再改松目标」0/5 全瞎** —— 而那正是最危险的一种。数据支持默认开，保留意见是负例构造偏易、n=20。`review_writes` 目前仍默认关闭 |

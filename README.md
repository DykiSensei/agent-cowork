...
M1 🔶 真实环境收口    1.1 ✅  1.2 ✅  1.3 ⏸ 待 API key  1.4 ✅
```
...
79 个测试。不起 Docker 的话，依赖真实服务的 14 个会 skip，其余照常跑；
另有 2 个真实供应商测试需要 `DEEPSEEK_API_KEY` 或 `MOONSHOT_API_KEY`。
...
管道已接通并验证到上游边界：请求经官方 SDK → LiteLLM 代理 → 供应商 API，
拿回带 `request_id` 的原生响应，只在鉴权处失败。无效 key 不会让 run 崩，
而是产出一条 `TOOL_FAILURE` 硬信号后挂起 —— 这条路径已测。

**待做**：给一把 key 后跑 demo，验证结构化输出的 schema 遵守度、架构师裁决质量、
以及模型分工是否合理。这些只能用真实模型跑出来。
...
| 项 | 状态 |
|---|---|
| M1.3 真实模型跑通 | ⏸ 等 API key |
| M2 参数实测 | `policy.py` 六个值仍是猜测。需先建 10–20 个任务的固定任务集 |

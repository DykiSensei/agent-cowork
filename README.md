...
M8 把复核接到了 `decide()` 的写入侧，但**默认关闭且判别力还没实测**
（`Architect(review_writes=True)` 才启用）。
...
| 项 | 状态 |
|---|---|
| **写入侧复核的判别力** | M8 出口 3：代码、用例表（11 个 / 6 种缺陷形态）、`cli bench-decide` 都就绪，**对照实验没跑过**。M7 那个 J 0.98 是在拆解上测的，不能假设迁移到「一次 spec 改动」上 —— 所以 `review_writes` 默认关闭。跑完出 TPR/FPR/J 才谈得上默认开 |

...
400 个测试。三样基础设施都不起时，依赖它们的 14 个会 skip（3 个 Postgres +
...
26 个用例上实测 J 0.963（deepseek 复核）/ FPR 0/24 —— 六种缺陷形态里四种 9/9、
两种 8/9。剩下那两处漏报（`limit_raised` / `scope_widened` 各 1/9）恰好是
**没有兜底**的那两种：调大上限之后任务接着跑，扩 scope 之后可以改考题、
任务会「成功」。漏得少，但漏了没人接（§11.19）。
...
| 项 | 状态 |
|---|---|
| **写入侧复核的默认开关** | M8 出口 5。实测四轮 321 次调用（§11.19），最终 26 个用例：**deepseek J 0.963 / kimi 0.907，FPR 两边都是 0/24**，复核者选 deepseek。数据支持默认开；`review_writes` 目前仍默认关闭，保留意见见 §11.19。**这个结论翻过一次** —— 11 个用例时看着是 kimi 更好，每种缺陷形态扩到 3 个用例后反了过来 |
